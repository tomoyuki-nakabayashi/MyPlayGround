fn main() {
    ...
    // A closure to print `color` which immediately borrows (`&`) `color` and stores
    // the borrow and closure in the `print` variable. It will remain borrowed until
    // `print` goes out of scope.
    ...
    // the closure leaving the original untouched.
